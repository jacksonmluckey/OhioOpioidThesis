...
OverdoseDeathRateCDC <- read_csv(here("data", "OverdoseDeathRate.csv")) %>%
  ...
  mutate(county = str_remove(county, " County, OH"),
         ...
         year = as.numeric(year)) # to enable merging with DF

# confirm where this is from:
...
if(!file.exists(here("data", "df.Rda"))) {
  ...
  df <- left_join(census, deaths) %>%
    left_join(OverdoseDeathRateCDC) %>%
    left_join(prescriptions)

  # convert OD raw to OD death rate per 10,000 pop
  df <- df %>%
