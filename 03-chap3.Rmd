...
# crude rate
crude_rate_individual_formula <- regression_formula("`Fatal Overdose Crude Rate`", "`Median Individual Income`", "Year")
crude_rate_individual_acs1 <- lm(crude_rate_individual_formula, df_acs1)
crude_rate_individual_acs5 <- lm(crude_rate_individual_formula, df_acs5)

# age adjusted rate
age_adjusted_rate_individual_formula <- regression_formula("`Fatal Overdose Age-Adjusted Rate`", "`Median Individual Income`", "Year")
age_adjusted_rate_individual_acs1 <- lm()
...
```{r crudeRateIndividualIncome, results="asis"}

stargazer(
  lm(crude_rate_individual, df_acs1),
  lm(crude_rate_individual, df_acs5),
  lm(crude_rate_individual_FE, df_acs1),
  lm(crude_rate_individual_FE, df_acs5),
  title = "Crude Rate on Individual Income with and without Year Fixed Effects",
  covariate.labels = c(
  ...
  omit = "Year",
  dep.var.caption = "Fatal Overdose Rate (Crude)",
...
```{r crudeRateHouseholdIncome, results="asis"}
crude_rate_household <- regression_formula("`Fatal Overdose Crude Rate`", "`Median Household Income`")
crude_rate_household_FE <- regression_formula("`Fatal Overdose Crude Rate`", "`Median Household Income`", "Year")
stargazer(lm(crude_rate_household, df_acs1),
          lm(crude_rate_household, df_acs5),
          lm(crude_rate_household_FE, df_acs1),
          lm(crude_rate_household_FE, df_acs5),
          covariate.labels = c(
    "Median Household Income",
    "Prescribing Rate",
    "Density: Small Metro",
    ...
    "\\% Disabled: Females 75 and Up"
          ...
          title = "Crude Fatal Overdose Rate on Household Income with and without Year Fixed Effects",
          dep.var.caption = "Fatal Overdose Rate (Crude)",
          omit = "Year",
          column.labels = c("ACS1", "ACS5", "ACS1 with FE", "ACS5 with FE"),
          add.lines = list(c("Fixed Effects?", "No", "No", "Yes", "Yes")),
...
```{r ageAdjustedRateIndividualIncome, results="asis"}
crude_rate_individual <- regression_formula("`Fatal Overdose Age-Adjusted Rate`", "`Median Individual Income`")
crude_rate_individual_FE <- regression_formula("`Fatal Overdose Age-Adjusted Rate`", "`Median Individual Income`", "Year")
stargazer(
  lm(crude_rate_individual, df_acs1),
  lm(crude_rate_individual, df_acs5),
  lm(crude_rate_individual_FE, df_acs1),
  lm(crude_rate_individual_FE, df_acs5),
  title = "Age-Adjusted Fatal Overdose Rate on Individual Income with and without Year Fixed Effects",
  covariate.labels = c(
    "Median Individual Income",
    "Prescribing Rate",
    "Density: Small Metro",
...
```{r ageAdjustedRateHouseholdIncome, results="asis"}
...
stargazer(lm(crude_rate_household, df_acs1),
    ...
    "\\% Disabled: Females 75 and Up"
          ...
          title = "Age-Adjusted Rate on Household Income with and without Year Fixed Effects",
          dep.var.caption = "Fatal Overdose Rate (Age-Adjusted)",
          omit = "Year",
          column.labels = c("ACS1", "ACS5", "ACS1 with FE", "ACS5 with FE"),
          add.lines = list(c("Fixed Effects?", "No", "No", "Yes", "Yes")),
...
```{r countyFixedEffects, results="asis"}
crude_rate_indivudal_FE <- regression_formula("`Fatal Overdose Age-Adjusted Rate`", "`Median Individual Income`", "County")
crude_rate_household_FE <- regression_formula("`Fatal Overdose Age-Adjusted Rate`", "`Median Household Income`", "County")
stargazer(lm(crude_rate_individual_FE, df_acs1),
          lm(crude_rate_individual_FE, df_acs5),
          lm(crude_rate_household_FE, df_acs1),
          lm(crude_rate_household_FE, df_acs5),
          covariate.labels = c(
    ...
    "\\% Disabled: Females 75 and Up"
          ...
          title = "Age-Adjusted Rate on Household Income with and without County Fixed Effects",
          dep.var.caption = "Fatal Overdose Rate (Age-Adjusted)",
          omit = "County",
          column.labels = c("ACS1", "ACS5", "ACS1 with FE", "ACS5 with FE"),
...
```{r allRateHouseholdIncome, results="asis"}
all_rate_household <- regression_formula("`Fatal Overdose Rate (Includes Imputed)`", "`Median Household Income`")
all_rate_household_FE <- regression_formula("`Fatal Overdose Rate (Includes Imputed)`", "`Median Household Income`", "Year")
stargazer(lm(all_rate_household, df_acs1),
          lm(all_rate_household, df_acs5),
          lm(all_rate_household_FE, df_acs1),
          lm(all_rate_household_FE, df_acs5),
          covariate.labels = c(
    "Median Household Income",
    "Prescribing Rate",
    "Density: Small Metro",
    ...
    "\\% Disabled: Females 75 and Up"
          ...
          title = "Fatal Overdose Rate (Including Imputed) on Household Income with and without Year Fixed Effects",
          dep.var.caption = "Fatal Overdose Rate (Including Imputed Values)",
          omit = "Year",
          column.labels = c("ACS1", "ACS5", "ACS1 with FE", "ACS5 with FE"),
          add.lines = list(c("Fixed Effects?", "No", "No", "Yes", "Yes")),
          single.row = TRUE,
